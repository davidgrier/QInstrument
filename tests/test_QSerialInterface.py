...
class TestOpen:
    ...
    @patch.object(QSerialInterface, 'setPortName')
    @patch.object(QSerialInterface, 'clear')
    @patch.object(QSerialInterface, 'close')
    @patch.object(QSerialInterface, 'identify', return_value=False)
    def test_failed_identify_closes_port(
            self, mock_id, mock_close, mock_clear, mock_set, iface):
        with patch.object(
                module.QtSP.QSerialPort, 'open', return_value=True):
            iface.open('ttyUSB0')
        mock_close.assert_called_once()

    @patch.object(QSerialInterface, 'setPortName')
    @patch.object(QSerialInterface, 'clear')
    @patch.object(QSerialInterface, 'identify', return_value=True)
    def test_successful_identify_leaves_port_open(
            self, mock_id, mock_clear, mock_set, iface):
        with patch.object(
                module.QtSP.QSerialPort, 'open', return_value=True), \
             patch.object(QSerialInterface, 'isOpen', return_value=True):
            result = iface.open('ttyUSB0')
...
class TestFind:
    ...
    def test_returns_self(self, iface):
        mock_port = MagicMock()
        mock_port.portName.return_value = 'ttyUSB0'
        with patch.object(
                module.QtSP.QSerialPortInfo, 'availablePorts',
                return_value=[mock_port]), \
             patch.object(QSerialInterface, 'open', return_value=True):
            result = iface.find()
        assert result is iface

    def test_stops_at_first_successful_port(self, iface):
        ports = [MagicMock(), MagicMock()]
        ports[0].portName.return_value = 'ttyUSB0'
        ports[1].portName.return_value = 'ttyUSB1'
        with patch.object(
                module.QtSP.QSerialPortInfo, 'availablePorts',
                return_value=ports), \
             patch.object(QSerialInterface, 'open',
                          return_value=True) as mock_open:
            iface.find()
        mock_open.assert_called_once_with('ttyUSB0')

    def test_tries_all_ports_on_failure(self, iface):
        ports = [MagicMock(), MagicMock()]
        ports[0].portName.return_value = 'ttyUSB0'
        ports[1].portName.return_value = 'ttyUSB1'
        with patch.object(
                ...
                return_value=ports), \
             patch.object(QSerialInterface, 'open',
                          return_value=False) as mock_open:
            iface.find()
        assert mock_open.call_count == 2

    def test_logs_error_when_no_port_found(self, iface, caplog):
        mock_port = MagicMock()
        mock_port.portName.return_value = 'ttyUSB0'
        with patch.object(
                module.QtSP.QSerialPortInfo, 'availablePorts',
                return_value=[mock_port]), \
             patch.object(QSerialInterface, 'open', return_value=False), \
